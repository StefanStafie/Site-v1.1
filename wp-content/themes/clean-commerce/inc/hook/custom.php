		    	...
		    	<div id="cart-section">
		    		<ul>
		    			<li class="account-login">
			    			<a href="<?php echo esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ); ?>"><?php echo is_user_logged_in() ? esc_html__( 'Contul meu', 'clean-commerce' ) : esc_html__( 'Contul meu', 'clean-commerce' ) ; ?></a>
			    			</li>
		    			<li class="cart-price"><a href="<?php echo esc_url( wc_get_cart_url() ); ?>"><strong><?php esc_html_e( 'Coș de cumpărături', 'clean-commerce' ) ?></strong>&nbsp;/&nbsp;<span class="amount"><?php echo WC()->cart->get_cart_total(); ?></span></a></li>
...
if ( ! function_exists( 'clean_commerce_footer_copyright' ) ) :
	...
	function clean_commerce_footer_copyright() {
		...
		// Powered by content.
		$powered_by_text = sprintf( esc_html__( 'ChicBarique by %s', 'clean-commerce' ), '<a target="_blank" rel="designer" href="http://facebook.com/stefan.stafie">' . esc_html__( ' MACThemes', 'clean-commerce' ) . '</a>' );
